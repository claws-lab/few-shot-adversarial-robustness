...
def iterative_run(dataprovider, eval_data, wrapper_config, train_eval_config, unlabeled_data=None, aug_data=None, output_dir=None):
    ...
    for gen in range(train_eval_config.generations):
        ...
        if not do_train:
            if gen < train_eval_config.generations - 1:
                continue
        ...
        if wrapper_config.arch_method in ['ipet', 'noisy_student'] and do_train:
            ...
            logger.info(f"Augmenting data by self-labeling unlabeled data of size {num_new_examples}.")
            if unlabeled_data is not None:
...
def main():
    ...
    with open(os.path.join("final_results", result_file_name), "a+") as f:
        if args.task_name in ["boolq", "wic", "wsc", "copa"]:
