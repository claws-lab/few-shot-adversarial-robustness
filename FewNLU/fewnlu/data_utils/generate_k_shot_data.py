...
def load_datasets(data_dir, tasks):
    ...
    for task in tasks:
        if task in ["MNLI", "MRPC", "QNLI", "QQP", "RTE", "SNLI", "SST-2", "STS-B", "WNLI", "CoLA"]:
            ...
            if task == "MNLI":
                splits = ["train", "dev_matched", "dev_mismatched", "dev_matched_adv", "dev_mismatched_adv", "test_matched", "test_mismatched"]
            elif task in ["SST-2", "QNLI", "QQP", "RTE"]:
                splits = ["train", "dev", "dev_adv", "test"]
            else: 
                splits = ["train", "dev"]
            for split in splits:
...
default=['SST-2', 'QQP', 'MNLI', 'QNLI', 'RTE'],
        help="Task names")
            ...
            if task in ["MNLI", "MRPC", "QNLI", "QQP", "RTE", "SNLI", "SST-2", "STS-B", "WNLI", "CoLA"]:
                ...
                for split, lines in dataset.items():
                    if split.startswith("train"):
                        ...
                        split = split.replace('test', 'unlabeled')
