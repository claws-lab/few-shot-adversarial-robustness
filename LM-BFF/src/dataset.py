...
def tokenize_multipart_input(
    ...
    # actual_mask_pos = input_ids.index(tokenizer.mask_token_id)
    # Padding
...
class FewShotDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, args, tokenizer, cache_dir=None, mode="train", use_demo=False, adv=False):
        self.args = args
        self.task_name = args.task_name + "_adv" if adv else args.task_name
        ...
        with FileLock(lock_path):
            ...
            if os.path.exists(cached_features_file) and not args.overwrite_cache:
                ...
                if mode == "dev":
                    ...
                    self.query_examples = self.processor.get_test_examples(args.data_dir, tag = "_adv" if adv else None)
                else:
        ...
        print(self.num_sample, len(self.query_examples))
    ...
    def __getitem__(self, i):
        if self.features is None:
            ...
            try:
                ...
                print(example)
                raise ValueError("tokenization error found")
        else:
