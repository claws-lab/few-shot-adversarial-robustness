...
from src.models import BertForPromptFinetuning, RobertaForPromptFinetuning, AlbertForPromptFinetuning, resize_token_type_embeddings
...
def main():
    ...
    test_adv_dataset = (
        FewShotDataset(data_args, tokenizer=tokenizer, mode="test", use_demo=("demo" in model_args.few_shot_type), adv=True)
        if training_args.do_predict
        else None
    )
    ...
    if training_args.do_predict:
        logging.info("*** Test ***")
        test_datasets = [test_dataset, test_adv_dataset]
        ...
        #     )
        #     test_datasets.append(
        #         FewShotDataset(mnli_mm_data_args, tokenizer=tokenizer, mode="test", use_demo=('demo' in model_args.few_shot_type), adv = True)
        #     )            
        ...
        for test_dataset in test_datasets:
            ...
            output_test_file = os.path.join(
                training_args.output_dir, f"test_results_{test_dataset.task_name}.txt"
            )
            if trainer.is_world_master():
                with open(output_test_file, "w") as writer:
                    logger.info("***** Test results {} *****".format(test_dataset.task_name))
                    for key, value in test_result.items():
                        logger.info("  %s = %s", key, value)
                        writer.write("%s = %s\n" % (key, value))
                        final_result[test_dataset.task_name + '_test_' + key] = value
